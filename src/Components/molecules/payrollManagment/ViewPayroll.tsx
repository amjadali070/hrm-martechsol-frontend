...
const ViewPayroll: React.FC = () => {
  ...
  const DetailRow = ({ label, value, isBold = false, isCurrency = false }: { label: string; value: string | number; isBold?: boolean; isCurrency?: boolean }) => (
    <div className="flex justify-between items-center py-2 border-b border-platinum-100 last:border-0 hover:bg-alabaster-grey-50 px-2 rounded transition-colors bg-white">
      ...
      <span className={`text-sm ${isBold ? 'font-bold text-gunmetal-900' : 'text-gunmetal-700'} ${isCurrency ? 'font-mono' : ''}`}>
        {value}
      </span>
    </div>
  );
  ...
  if (loading && !payroll) {
    return (
      <div className="flex flex-col items-center justify-center h-[80vh]">
        ...
        <p className="text-slate-grey-500 font-medium text-sm animate-pulse">Loading Payroll...</p>
      </div>
    );
  }
  ...
  if (!loading && !payroll) {
     return (
        <div className="flex flex-col items-center justify-center h-[80vh] text-center p-6">
          ...
          <h3 className="text-xl font-bold text-gunmetal-900">Payroll Not Found</h3>
          <p className="text-slate-grey-500 mt-2 mb-6">The requested payroll record is not available.</p>
          <button
    ...
    <div className="min-h-screen bg-alabaster-grey-50 p-6 md:p-12">
       <div className="max-w-6xl mx-auto">
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
               <div>
                  <h1 className="text-3xl font-extrabold text-gunmetal-900 tracking-tight">Payroll Details</h1>
                  <p className="text-slate-grey-500 mt-1">
                      Viewing record for <span className="font-semibold text-gunmetal-700">{getMonthName(payroll!.month)} {payroll!.year}</span>
                  </p>
               </div>
                  ...
                  >
                    {({ loading }) => (
                         <>
                            ...
                            {loading ? "Preparing PDF..." : "Download Play Slip"}
                         </>
                    )}
                  </PDFDownloadLink>
               </div>
               ...
               <div className="lg:col-span-2 space-y-8">
                   ...
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-platinum-200 relative overflow-hidden">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                           <DetailRow label="Employee Name" value={payroll!.user.name} isBold={true} />
                           <DetailRow label="Department" value={payroll!.user.personalDetails?.department} />
                           <DetailRow label="Job Title" value={payroll!.user.personalDetails?.jobTitle || "N/A"} />
                   ...
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-platinum-200">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
                           <DetailRow label="Basic Salary" value={formatCurrency(payroll!.basicSalary)} isCurrency={true} />
                           <DetailRow label="Medical Allowance" value={formatCurrency((payroll as any)?.user?.salaryDetails?.medicalAllowance || payroll!.allowances)} isCurrency={true} />
                           <DetailRow label="Mobile Allowance" value={formatCurrency((payroll as any)?.user?.salaryDetails?.mobileAllowance || 0)} isCurrency={true} />
                           <DetailRow label="Fuel Allowance" value={formatCurrency((payroll as any)?.user?.salaryDetails?.fuelAllowance || 0)} isCurrency={true} />
                           <DetailRow label="Per Day Rate" value={formatCurrency(payroll!.perDaySalary)} isCurrency={true} />
                           <div className="md:col-span-2 bg-emerald-50 rounded-lg p-3 flex justify-between items-center mt-2 border border-emerald-100">
                               <span className="font-bold text-emerald-800">Total Earnings (Gross)</span>
                               <span className="font-mono font-bold text-emerald-700 text-lg">{formatCurrency(payroll!.totalSalary)}</span>
                           </div>
                       </div>
                   </div>
                   ...
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-platinum-200">
                        <SectionTitle icon={FaRegCalendarCheck} title="Attendance & Leaves" />
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                            <div className="bg-alabaster-grey-50 p-3 rounded-xl border border-platinum-100 text-center">
                                <span className="block text-2xl font-bold text-gunmetal-900">{payroll?.absentDates?.length || 0}</span>
                                <span className="text-xs text-slate-grey-500 uppercase font-bold">Absent Days</span>
                            </div>
                             <div className="bg-alabaster-grey-50 p-3 rounded-xl border border-platinum-100 text-center">
                                <span className="block text-2xl font-bold text-gunmetal-900">{payroll?.lateIns || 0}</span>
                                <span className="text-xs text-slate-grey-500 uppercase font-bold">Late Arrivals</span>
                            </div>
                             <div className="bg-alabaster-grey-50 p-3 rounded-xl border border-platinum-100 text-center">
                                <span className="block text-2xl font-bold text-gunmetal-900">{payroll?.halfDays || 0}</span>
                                <span className="text-xs text-slate-grey-500 uppercase font-bold">Half Days</span>
                            </div>
                        </div>

                         {payroll?.absentDates && payroll.absentDates.length > 0 && (
                             <div className="mt-4">
                                 <p className="text-xs font-bold text-slate-grey-400 uppercase mb-2">Absent Dates Log</p>
                                 <div className="flex flex-wrap gap-2">
                                     {payroll.absentDates.map((date, i) => (
                                         <span key={i} className="px-2 py-1 bg-red-50 text-red-600 rounded text-xs border border-red-100 font-mono">
               ...
               <div className="flex flex-col gap-8">
                    ...
                    <div className="bg-white p-6 rounded-2xl shadow-sm border border-platinum-200">
                        ...
                        <div className="space-y-1">
                            <DetailRow label="Absent Deductions" value={formatCurrency((payroll?.absentDates?.length || 0) * (payroll?.perDaySalary || 0))} isCurrency={true} />
                            <DetailRow label="Late Deductions" value={formatCurrency((Math.floor((payroll?.lateIns ?? 0) / 4) * (payroll?.perDaySalary ?? 0)) / 2)} isCurrency={true} />
                            <DetailRow label="Half Day Deductions" value={formatCurrency(((payroll?.halfDays || 0) * (payroll?.perDaySalary || 0)) / 2)} isCurrency={true} />
                            <div className="my-2 border-t border-platinum-100"></div>
                            <DetailRow label="Income Tax" value={formatCurrency(tax)} isCurrency={true} />
                            <DetailRow label="EOBI" value={formatCurrency(eobi)} isCurrency={true} />
                            <DetailRow label="Provident Fund" value={formatCurrency(employeePF)} isCurrency={true} />
                            
                            <div className="bg-rose-50 rounded-lg p-3 flex justify-between items-center mt-3 border border-rose-100">
                               <span className="font-bold text-rose-800">Total Deductions</span>
                               <span className="font-mono font-bold text-rose-700 text-lg">-{formatCurrency(payroll!.deductions + tax + eobi + employeePF)}</span>
                           </div>
                        </div>
                    </div>
                             ...
                             <div className="space-y-2">
                                 {extraPayments.map(pay => (
                                     <div key={pay.id} className="flex justify-between items-center text-sm">
                                         <span className="text-gunmetal-700">{pay.description}</span>
                                         <span className="font-mono font-bold text-emerald-600">+{formatCurrency(pay.amount)}</span>
                                     </div>
                                 ))}
                             </div>
                        </div>
                    ...
                    <div className="bg-gunmetal-900 p-6 rounded-2xl shadow-xl text-white">
                        <div className="flex items-center gap-3 mb-4 opacity-80">
                            <FaUniversity />
                            <span className="text-sm font-bold uppercase tracking-widest">Net Payable</span>
                        </div>
                        ...
                        <p className="text-sm text-gunmetal-300">
                            This is the final amount to be transferred to the employees account after all adjustments.
                        </p>
                    </div>
