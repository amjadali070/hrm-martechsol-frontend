            ...
            {payrolls.length > 0 ? (
                <div className="overflow-hidden rounded-xl border border-platinum-200 shadow-sm bg-white">
                 ...
                 <div className="flex flex-col items-center justify-center py-20 bg-alabaster-grey-50/30 rounded-2xl border-2 border-dashed border-platinum-200">
                    ...
                    <p className="text-slate-grey-500 text-sm mt-1">Salary slips will appear here once generated.</p>
                </div>
            )}
        </div>
      </div>
         ...
         <div className="bg-white px-8 py-6 border-b border-platinum-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
           <div className="flex items-center gap-4">
              ...
              <div>
                  ...
                  <p className="text-sm font-medium text-slate-grey-500 flex items-center gap-2">
                    <span className="font-bold text-gunmetal-700">{getMonthName(selectedPayroll.month)} {selectedPayroll.year}</span>
           ...
           <div className="flex items-center gap-3 w-full md:w-auto">
               <button
                ...
                >
                    {({ loading }) => (
                         loading ? 'Generating...' : <><FaDownload size={14} /> Download PDF</>
                     )}
                </PDFDownloadLink>
           </div>
        </div>
      ...
      <div className="p-8 space-y-8 bg-alabaster-grey-50/30">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-6 border border-platinum-200 rounded-2xl shadow-sm relative overflow-hidden group">
                ...
                <div className="space-y-4 relative z-10">
                    <div>
                        <p className="text-xs text-slate-grey-500 font-bold uppercase mb-1">Full Name</p>
                        <p className="text-lg font-bold text-gunmetal-900">{selectedPayroll?.user.name}</p>
                    </div>
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                             <p className="text-xs text-slate-grey-500 font-bold uppercase mb-1">Job Title</p>
                             <p className="text-sm font-bold text-gunmetal-700">{selectedPayroll?.user.personalDetails?.jobTitle || "N/A"}</p>
                        </div>
                        <div>
                             <p className="text-xs text-slate-grey-500 font-bold uppercase mb-1">Department</p>
                             <p className="text-sm font-bold text-gunmetal-700">{selectedPayroll?.user.personalDetails?.department}</p>
                        </div>
                     </div>
                     <div>
                        <p className="text-xs text-slate-grey-500 font-bold uppercase mb-1">Email Address</p>
                        <p className="text-sm font-medium text-slate-grey-600">{selectedPayroll?.user.email}</p>
                    </div>
                </div>
            </div>
                  ...
                  <div className="space-y-4 relative z-10">
                     <div className="flex items-center gap-4">
                         <div className="p-3 bg-gunmetal-50 rounded-xl text-center min-w-[80px] border border-platinum-100">
                             <span className="block text-2xl font-bold text-gunmetal-900">{selectedPayroll!.year}</span>
                             <span className="text-xs font-bold text-slate-grey-500 uppercase">Year</span>
                         </div>
                         <div className="p-3 bg-gunmetal-50 rounded-xl text-center min-w-[100px] border border-platinum-100 flex-1">
                             <span className="block text-2xl font-bold text-gunmetal-900">{getMonthName(selectedPayroll!.month)}</span>
                             <span className="text-xs font-bold text-slate-grey-500 uppercase">Month</span>
                         </div>
                     </div>
                     <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 mt-2">
                         <div className="flex justify-between items-center">
                             <span className="text-emerald-800 font-bold text-sm">Payment Status</span>
                             <span className="bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide">Processed</span>
                         </div>
                     </div>
                </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Earnings */}
             <div className="bg-white border border-platinum-200 rounded-2xl overflow-hidden shadow-sm flex flex-col h-full">
                <div className="bg-alabaster-grey-50 px-6 py-5 border-b border-platinum-200 flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-gunmetal-900 uppercase tracking-wide">Earnings Breakdown</h3>
                </div>
                <div className="p-6 space-y-4 flex-1">
                    <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Basic Salary</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{formatCurrency(selectedPayroll!.basicSalary.toFixed(0))}</span>
                    </div>
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Medical Allowance</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{ (selectedPayroll as any)?.user?.salaryDetails?.medicalAllowance || selectedPayroll!.allowances}</span>
                    </div>
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Mobile Allowance</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{ (selectedPayroll as any)?.user?.salaryDetails?.mobileAllowance || 0}</span>
                    </div>
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Fuel Allowance</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{ (selectedPayroll as any)?.user?.salaryDetails?.fuelAllowance || 0}</span>
                    </div>
                    {/* Extra Payments */}
                    {extraPayments.map((payment) => (
                         <div key={payment.id} className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                            <span className="text-sm font-medium text-slate-grey-600 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                                {payment.description}
                            </span>
                            <span className="text-sm font-bold text-gunmetal-900 font-mono">{formatCurrency(payment.amount.toFixed(0))}</span>
                        </div>
                    ))}
                </div>
                <div className="bg-emerald-50/50 p-6 border-t border-platinum-200 mt-auto">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-lg font-bold text-emerald-600 font-mono">{formatCurrency(selectedPayroll!.totalSalary.toFixed(0))}</span>
                    </div>
                </div>
             </div>
            ...
            <div className="bg-white border border-platinum-200 rounded-2xl overflow-hidden shadow-sm flex flex-col h-full">
                <div className="bg-alabaster-grey-50 px-6 py-5 border-b border-platinum-200 flex items-center gap-3">
                    ...
                    <h3 className="text-sm font-bold text-gunmetal-900 uppercase tracking-wide">Deductions Breakdown</h3>
                </div>
                <div className="p-6 space-y-4 flex-1">
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Tax</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{formatCurrency(tax.toFixed(0))}</span>
                    </div>
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">EOBI</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{formatCurrency(eobi.toFixed(0))}</span>
                    </div>
                    <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Provident Fund</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">{formatCurrency(employeePF.toFixed(0))}</span>
                    </div>
                    {/* Attendance Based Deductions Calculation Display */}
                     <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Absent Deductions <span className="text-xs font-bold text-rose-500 bg-rose-50 px-1.5 py-0.5 rounded ml-1">{selectedPayroll?.absentDates?.length || 0} days</span></span>
                    ...
                    <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Late IN Deductions</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">
                             {formatCurrency(
                    ...
                    <div className="flex justify-between items-center pb-3 border-b border-platinum-100 border-dashed">
                        <span className="text-sm font-medium text-slate-grey-600">Half Day Deductions</span>
                        <span className="text-sm font-bold text-gunmetal-900 font-mono">
                             {formatCurrency(
                ...
                <div className="bg-rose-50/50 p-6 border-t border-platinum-200 mt-auto">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-lg font-bold text-rose-600 font-mono">{formatCurrency((selectedPayroll!.deductions + tax + eobi + employeePF).toFixed(0))}</span>
                    </div>
                </div>
             </div>
        </div>
        ...
        <div className="bg-gunmetal-900 rounded-2xl p-8 md:p-10 text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10 text-center md:text-left mb-6 md:mb-0">
                 <h2 className="text-3xl font-bold mb-2 tracking-tight">Net Salary Payable</h2>
                 <p className="text-platinum-400 text-sm font-medium">Final amount to be transferred to employee bank account</p>
            </div>
                 ...
                 {selectedPayroll?.absentDates && selectedPayroll.absentDates.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                         {selectedPayroll.absentDates.map((date, idx) => (
                             <span key={idx} className="px-2.5 py-1.5 bg-rose-50 border border-rose-100 text-rose-700 text-xs font-bold rounded-lg font-mono">
                 ...
                 {selectedPayroll?.lateInDates && selectedPayroll.lateInDates.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                         {selectedPayroll.lateInDates.map((date, idx) => (
                             <span key={idx} className="px-2.5 py-1.5 bg-amber-50 border border-amber-100 text-amber-700 text-xs font-bold rounded-lg font-mono">
                                 {new Date(date).toLocaleDateString()}
                             </span>
                         ))}
                    </div>
                 ...
                 {selectedPayroll?.halfDayDates && selectedPayroll.halfDayDates.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                         {selectedPayroll.halfDayDates.map((date, idx) => (
                             <span key={idx} className="px-2.5 py-1.5 bg-orange-50 border border-orange-100 text-orange-700 text-xs font-bold rounded-lg font-mono">
                                 {new Date(date).toLocaleDateString()}
                             </span>
                         ))}
                    </div>
