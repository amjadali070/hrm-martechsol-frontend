...
const FeedbackForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!message.trim() || message === '<p><br></p>') newErrors.message = 'Message is required.';
    setErrors(newErrors);
      ...
      <div className="flex flex-col p-8 w-full bg-white rounded-2xl shadow-md">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex justify-end mt-5">
            <button
              type="submit"
              className={`px-6 py-3 font-semibold text-white bg-purple-900 rounded-2xl transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
